...
const FIREBASE_API_KEY = 'AIzaSyC64NnAB0rH9Ne5gFINhaFSbqkJ4ygYZfY';
...
async function writeItemsToFirestore(items) {
  return new Promise(resolve => {
    chrome.storage.local.get(['typhon-auth'], async data => {
      const auth = data['typhon-auth'];
      if (!auth || !auth.uid || (!auth.idToken && !auth.refreshToken)) { resolve(false); return; }
      try {
        let idToken = auth.idToken;
        const expiry = auth.expiresAt || 0;
        if (Date.now() >= expiry - 60000 && auth.refreshToken) {
          ...
          if (r.ok) {
            ...
            idToken = t.id_token;
            chrome.storage.local.set({ 'typhon-auth': Object.assign({}, auth, {
              idToken, refreshToken: t.refresh_token || auth.refreshToken,
              expiresAt: Date.now() + (parseInt(t.expires_in, 10) || 3600) * 1000
            }) });
          }
        }
        const url = `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT}/databases/(default)/documents/typhonCases/${auth.uid}?updateMask.fieldPaths=typhon-items`;
        const body = { fields: { 'typhon-items': jsToFirestoreValue(items) } };
...
async function refreshItemsFromFirestore(tabId) {
  return new Promise(resolve => {
    chrome.storage.local.get(['typhon-auth', 'typhon-items'], async data => {
      const oldItems = data['typhon-items'] || [];
      ...
      try {
        let idToken = auth.idToken;
        // Refresh token if expired (exp is seconds since epoch)
        const expiry = auth.expiresAt || 0;
        if (Date.now() >= expiry - 60000 && auth.refreshToken) {
          const r = await fetch(TOKEN_URL, {
            method: 'POST',
          ...
          if (r.ok) {
            ...
            chrome.storage.local.set({ 'typhon-auth': updatedAuth });
          }
        }
        ...
        if (tabId) {
          chrome.tabs.sendMessage(tabId, { action: 'showToast', message: '⚠️ Typhon Case Helper couldn\'t auto-update. Click the extension icon to get the latest items.' });
        }
      }
      resolve(false);
    });
