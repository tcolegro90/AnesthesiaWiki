...
async function saveCase() {
  ...
  // Tag with CPG plan name if this case came from a planned case pre-fill
  if (window._activePlanName) c.cpgPlanName = window._activePlanName;
  ...
  if (writeResult?.cloud) {
    ...
    if (typeof window.markNeedsCloudRetry === 'function') window.markNeedsCloudRetry();
    toast('Saved locally. Cloud sync failed.');
  } else {
    hideCloudSyncWarning();
    toast('Case saved locally (not signed in)');
  }
  clearDraft({ clearCloud: true });
  resetCase();
  showStartScreen();
}
...
async function saveTimeLog() {
  ...
  resetTimeLog();
  clearTimeProgress({ clearCloud: true });
  showStartScreen();
}
...
async function saveEval() {
  ...
  clearEvalProgress({ clearCloud: true });
  resetEval();
  showStartScreen();
}
