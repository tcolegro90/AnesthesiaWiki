...
function saveDraft() {
  try {
    const data = JSON.stringify(readCase());
    localStorage.setItem('typhon-draft', data);
    localStorage.setItem('typhon-case-progress-updated', String(Date.now()));
    clearTimeout(_draftCloudTimer);
    _draftCloudTimer = setTimeout(() => store.set('typhon-draft', JSON.parse(data)).catch(() => {}), 3000);
  } catch(e) {}
}
function clearDraft() {
  clearTimeout(_draftCloudTimer);
  ...
  store.set('typhon-draft', null).catch(() => {});
}
function saveTimeProgress() {
  try {
    const data = JSON.stringify(readTimeLog());
    localStorage.setItem('typhon-time-progress', data);
    localStorage.setItem('typhon-time-progress-updated', String(Date.now()));
    clearTimeout(_timeCloudTimer);
    _timeCloudTimer = setTimeout(() => store.set('typhon-time-progress', JSON.parse(data)).catch(() => {}), 3000);
  } catch(e) {}
}
function clearTimeProgress() {
  ...
  store.set('typhon-time-progress', null).catch(() => {});
}
function saveEvalProgress() {
  try {
    const data = JSON.stringify(readEval());
    localStorage.setItem('typhon-eval-progress', data);
    localStorage.setItem('typhon-eval-progress-updated', String(Date.now()));
    clearTimeout(_evalCloudTimer);
    _evalCloudTimer = setTimeout(() => store.set('typhon-eval-progress', JSON.parse(data)).catch(() => {}), 3000);
  } catch(e) {}
}
function clearEvalProgress() {
  ...
  store.set('typhon-eval-progress', null).catch(() => {});
}
