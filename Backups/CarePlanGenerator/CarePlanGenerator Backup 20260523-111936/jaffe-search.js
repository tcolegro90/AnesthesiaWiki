    ...
    badge.innerHTML =
      pages.map(function(p) {
        return `<span style="display:flex;align-items:center;width:100%;">` +
          `<button type="button" onclick="removeJaffePage(${p})" title="Remove" style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;flex-shrink:0;border-radius:50%;background:#b08060;color:white;border:none;cursor:pointer;font-size:14px;font-weight:900;line-height:1;margin-right:5px;padding:0;">&times;</button>` +
          `<button type="button" onclick="openJaffePage(${p})" style="flex:1;text-align:left;background:none;border:none;padding:0;font:inherit;color:inherit;cursor:pointer;">📖 Jaffe 6th Ed p.${p}</button>` +
          `</span>`;
      }).join('');
