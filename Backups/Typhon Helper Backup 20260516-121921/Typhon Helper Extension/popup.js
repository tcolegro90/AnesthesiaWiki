...
async function signInWithGoogle() {
  ...
  await new Promise(r => chrome.storage.local.set({
    'typhon-auth': { uid, email, idToken, tokenTs: Date.now(), refreshToken }
  }, r));
  ...
  if (auth && auth.refreshToken) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        await new Promise(r => chrome.storage.local.set({
          'typhon-auth': { ...auth, idToken: newToken, tokenTs: Date.now(), refreshToken: newRefresh }
        }, r));
...
async function init() {
  ...
  } else {    pendingItems = allItems.filter(i => !i.submitted);
  }
