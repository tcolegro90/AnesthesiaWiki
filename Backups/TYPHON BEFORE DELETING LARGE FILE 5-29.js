...
(async () => {
  ...
  if (typeof window.updateContinueButton === 'function') window.updateContinueButton();
  const _vBadge = document.getElementById('ext-version-badge');
  if (_vBadge && typeof chrome !== 'undefined' && chrome.runtime?.getManifest) {
    _vBadge.textContent = 'v' + chrome.runtime.getManifest().version;
  }
  const items = (await store.get('typhon-items')) || [];
  updateBadge(items);
