        ...
        try {
          ...
          if (resp.ok) {
            ...
            if (rawField?.arrayValue?.values) {
              ...
              // If the merge added any submitted flags that Firestore didn't have,
              // write them back to Firestore now (using the already-valid idToken).
              const anyNewlySubmitted = merged.some((item, i) => item.submitted && !items[i].submitted);
              if (anyNewlySubmitted) {
                writeItemsToFirestore(merged, idToken, auth.uid).catch(() => {});
              }
...
// Write items array back to Firestore using an already-valid idToken.
async function writeItemsToFirestore(items, idToken, uid) {
  ...
  const body = JSON.stringify({
    fields: { 'typhon-items': { arrayValue: { values: items.map(jsValueToFirestore) } } }
  });
...
function jsValueToFirestore(val) {
  ...
  if (Array.isArray(val)) return { arrayValue: { values: val.map(jsValueToFirestore) } };
  if (typeof val === 'object') {
    const fields = {};
    for (const [k, v] of Object.entries(val)) fields[k] = jsValueToFirestore(v);
