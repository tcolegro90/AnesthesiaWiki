...
var store = {
    ...
    if (db && uid) {
      try {
        ...
        if (snap.exists) {
          ...
          // Safety check: if localStorage has MORE typhon-items than Firestore,
          // prior cloud writes likely failed and local is more current — use it.
          if (key === 'typhon-items' && Array.isArray(data)) {
            let local = null;
            try { local = JSON.parse(localStorage.getItem(key)); } catch {}

            if (Array.isArray(local) && local.length > data.length) {
              console.warn('[store.get] localStorage has', local.length, 'items vs Firestore', data.length, '— using localStorage (prior cloud writes may have failed)');
              return local;
            }

            // Preserve locally-recorded submitted flags if cloud data is briefly stale.
            if (Array.isArray(local) && local.length) {
              const localSubmittedIds = new Set(
                local
        ...
        // Firestore doc doesn't exist yet — fall through to localStorage below.
  ...
  async set(key, val) {
    ...
    const uid = _getAuthUid();
    if (db && uid) {
      // Write locally first so localStorage is always current, even if Firestore is slow.
