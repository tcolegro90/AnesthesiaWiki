      ...
      try {
        ...
        if (u && typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
          ...
          await new Promise(r => chrome.storage.local.set({
            'typhon-auth': { uid: u.uid, email: u.email || '', idToken: tok, tokenTs: Date.now(), refreshToken: u.refreshToken }
          }, r));
            ...
            if (stored.refreshToken) {
              ...
              if (r.ok) {
                ...
                chrome.storage.local.set({ 'typhon-auth': Object.assign({}, stored, {
                  idToken: t.id_token, refreshToken: t.refresh_token || stored.refreshToken, tokenTs: Date.now()
                }) });
              }
            }
          } catch (e) {
        ...
        try {
          ...
          await new Promise(resolve => {
            chrome.storage.local.set(
              { 'typhon-auth': { uid: user.uid, email: user.email || '', idToken, tokenTs: Date.now(), refreshToken: user.refreshToken } },
              () => {
