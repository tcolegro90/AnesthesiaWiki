...
/* global mirroredState, getState */
    ...
    function buildPrintCard(overrideState) {
      ...
      if (bloodRoomRow) {
        if (!!s['pat-blood-room-yes']) {
          bloodRoomRow.style.display = 'flex';
          bloodRoomRow.style.color = '#c41c3b';
          bloodRoomRow.style.fontWeight = 'bold';
        } else {
      ...
      var interpreterAlertRow = document.getElementById('pr-interpreter-alert-row');
      if (interpreterAlertRow) {
        if (interpreterYes) {
          var lang = (s['pat-interpreter-language'] || '').trim();
          setText('pr-interpreter-alert-text', lang ? (lang + ' Speaking') : 'Interpreter Needed');
          interpreterAlertRow.style.display = 'flex';
        } else {
          interpreterAlertRow.style.display = 'none';
        }
      }
      ...
      if (tsGoodUntilRow) {
        ...
        if (tsExpiryMs !== null) {
          if (tsExpiryMs < refMs + 6 * 60 * 60 * 1000) {
            ...
            // Comfortably valid: green
            tsColor = '#1a6e2e'; tsWeight = 'bold';
          }
        }
      ...
      ['awareness','mh','pseudo'].forEach(function(k) {
        var on = !!s['hx-' + k + '::Yes'];
        var row = document.getElementById('pr-row-' + k);
        if (row) row.style.display = on ? 'flex' : 'none';
      });
      // Only show generic "Fam Hx Anes Comp" if none of the specific conditions (MH, PChE, Awareness) are already shown
      var famhxOn = !!s['hx-famhx::Yes'];
      var specificShown = !!s['hx-mh::Yes'] || !!s['hx-pseudo::Yes'] || !!s['hx-awareness::Yes'];
      var famhxRow = document.getElementById('pr-row-famhx');
      if (famhxRow) famhxRow.style.display = (famhxOn && !specificShown) ? 'flex' : 'none';
      var ponvRow = document.getElementById('pr-row-ponv');
      if (ponvRow) ponvRow.style.display = !!s['hx-ponv::Yes'] ? 'flex' : 'none';
      var metsRow = document.getElementById('pr-mets-row');
      if (metsRow) metsRow.style.display = s['hx-mets::Yes'] ? 'flex' : 'none';
      ...
      if (atlExam === 'Limited Mobility') {
        setText('pr-abn-atl', 'Limited atlantooccipital mobility');
      ...
      var mpRow = document.getElementById('pr-mallampati-row');
      if (!isNaN(mpExam) && mpExam > 2) {
        if (mpRow) { mpRow.style.color = '#c41c3b'; mpRow.style.fontWeight = 'bold'; }
      } else {
        if (mpRow) { mpRow.style.color = ''; mpRow.style.fontWeight = ''; }
      }
      ...
      var rsiYes = !!s['ind-rsi::Yes'];
      var airwayEl = document.getElementById('pr-airway');
      var airwayLabelEl = document.getElementById('pr-airway-label');
      if (airwayLabelEl) airwayLabelEl.textContent = 'Airway:';
      if (airwayEl) {
        if (rsiYes) {
          airwayEl.innerHTML = escapeHtml(airwayMethod) + ' / <span style="color:#c41c3b;font-weight:bold;">RSI</span>';
        } else {
          airwayEl.textContent = airwayMethod;
        }
      ...
      var printNameMap = {
        'Propofol Gtt - Light':    'Prop Gtt',
        'Propofol Gtt - Moderate': 'Prop Gtt',
        'Propofol Gtt - Deep':     'Prop Gtt',
        'Remifentanil infusion':        'Remi Gtt',
        'Ketamine (sub-dissociative)':  'Ketamine',
        'Dexmedetomidine (bolus)':      'Precedex',
        'Dexmedetomidine (infusion)':   'Precedex',
        'Dexmedetomidine':              'Precedex'
      };
      var sedDrip = (s['ind-sed-drip'] || '').trim();
      var sedDripRow = document.getElementById('pr-sed-drip-row');
      if (sedDripRow) {
        if (sedDrip) {
          var sedDripDose = (s['ind-sed-drip-dose'] || '').trim();
          var sedDripHints = {
            'Propofol Gtt - Light':    '25-75 mcg/kg/min',
            'Propofol Gtt - Moderate': '75-125 mcg/kg/min',
            'Propofol Gtt - Deep':     '125-200 mcg/kg/min'
          };
          var doseStr = sedDripDose || sedDripHints[sedDrip] || '';
          setText('pr-sed-drip', (printNameMap[sedDrip] || sedDrip) + (doseStr ? ' (' + doseStr + ')' : ''));
          sedDripRow.style.display = 'flex';
        } else {
          sedDripRow.style.display = 'none';
        }
      }
      ...
      var painUnitMap = {
        ...
        'Dexmedetomidine': 'mcg/kg/hr', 'Dexmedetomidine (bolus)': 'mcg/kg', 'Dexmedetomidine (infusion)': 'mcg/kg/hr', 'Lidocaine infusion': 'mg/kg/hr',
      ...
      var postPainRow = document.getElementById('pr-pain-post-row');
      if (postPainRow) postPainRow.style.display = painPostList.length ? 'flex' : 'none';
      ...
      var isNonSmoker = !s['pat-smoker::Yes'];
      ...
      var equipLabels = {
        ...
        'equip-bite-block': 'Bite block',
        'equip-microbore-tubing': 'Microbore tubing',
      ...
      var ibwVal = s['pat-ibw'] ? parseFloat(s['pat-ibw']) : null;
      if (!ibwVal && !isNaN(totalInches) && totalInches > 0 && (s['pat-gender'] === 'M' || s['pat-gender'] === 'F')) {
        var refIn2 = totalInches < 60 ? 60 : totalInches;
