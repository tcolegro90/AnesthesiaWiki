...
function initYNToggles() {
  ...
  yesInputs.forEach(function(yesInput) {
    ...
    cb.addEventListener('change', function() {
      ...
      stateLabel.textContent = cb.checked ? 'Yes' : 'No';
      // Fire change event on the newly-checked radio so page logic responds
      var evt = new Event('change', { bubbles: true });
      (cb.checked ? yesInput : noInput).dispatchEvent(evt);
    });

    // Radio → toggle sync (handles programmatic state restores)
...
// Re-sync toggle visual state after external state restores (postMessage updates)
...
function buildSideNav() {
  ...
  var pages = [
    ...
    { label: 'Labs',              file: '2-Labs-Chemistries.html' },
    { label: 'PMH / PSH',         file: '3-PMH.html' },
    { label: 'Meds',              file: '4-Prescribed-Medications.html' },
    { label: 'Preop History',     file: '5-Preop-History.html' },
    { label: 'Airway',            file: '6-Airway-Exam.html' },
    { label: 'Extras',            file: '6b-Extras.html' },
    { label: 'Plan',              file: '7-Anesthetic-Plan.html' },
    { label: 'PONV',              file: '7b-APFEL-PONV.html' },
    { label: 'Fluid / Blood',     file: '8-Fluid-Blood-Plan.html' }
  ];
